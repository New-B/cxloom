...
class TokenService {
  public:
    ...
    // Serializes token producers/handlers with local retirement closure.
    bool CloseObject(GlobalPointer object);
    Status ProgressOutbound();
  ...
  private:
    ...
    Status EnqueueOutbound(HostId destination, QueueEnvelope envelope);
    ...
    mutable std::recursive_mutex protocol_mutex_;
    std::array<std::deque<QueueEnvelope>, kMaxHosts> outbound_;
